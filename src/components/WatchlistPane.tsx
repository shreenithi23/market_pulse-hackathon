  ...
  return (
    <div className="flex-1 p-4 max-w-7xl mx-auto w-full font-body">
      ...
      {triggeredReminders.length > 0 && (
        <div className="mb-6 card-neu p-4.5 bg-[#E0E5EC] border-2 border-[#38B2AC]/60 rounded-3xl animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1">
                  {triggeredReminders.map(r => (
                    <span
                      ...
                      className="cursor-pointer font-mono font-bold text-xs text-[#2C7A7B] bg-[#E0E5EC] shadow-neu-inset-sm px-2 py-0.5 rounded-xl hover:text-[#6C63FF] transition-colors"
                    >
                      {r.symbol}: Hit {r.targetBuyCurrency === 'INR' ? '₹' : '$'}{r.targetBuyPrice.toLocaleString()} (Now {r.targetBuyCurrency === 'INR' ? '₹' : '$'}{r.priceInTargetCurrency.toLocaleString()})
                    </span>
                  ))}
                </div>
              </div>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-[#D1D9E6]">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-display font-bold uppercase tracking-wider rounded-2xl transition-all duration-300 ${
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-display font-bold uppercase tracking-wider rounded-2xl transition-all duration-300 flex items-center gap-1.5 ${
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="bg-[#E0E5EC] shadow-neu-inset rounded-2xl p-1 flex items-center gap-1">
            <button
          ...
          <div className="relative">
            <Search className="absolute left-3.5 top-2.5 h-3.5 w-3.5 text-[#6B7280]" strokeWidth={2.2} />
            <input
              type="text"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Search watchlist..."
              className="bg-[#E0E5EC] shadow-neu-inset rounded-2xl py-2 pl-9 pr-4 text-xs font-medium text-[#3D4852] placeholder-[#A0AEC0] focus:shadow-neu-inset-deep focus:outline-none w-40 sm:w-52"
            />
          </div>
          ...
          <button
            ...
            className="btn-neu-primary px-4 py-2 text-xs font-bold rounded-2xl flex items-center gap-2 hover:-translate-y-0.5 active:translate-y-0.5 transition-all duration-300"
          >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-start gap-3.5 flex-wrap flex-1 min-w-[240px]">
                    ...
                    <div>
                      ...
                      {hasBuyTarget ? (
                        <div className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl shadow-neu-inset-sm text-[11px] font-mono font-bold">
                          {isTargetMet ? (
                            <>
                              <CheckCircle2 className="h-3.5 w-3.5 text-[#38B2AC] shrink-0 animate-pulse" />
                              <span className="text-[#2C7A7B]">
                                Target Reached: ≤ {targetCurrency === 'INR' ? '₹' : '$'}{thresh.targetBuyPrice?.toLocaleString()}
                              </span>
                            </>
                          ) : (
                            <>
                              ...
                              <span className="text-[#6B7280]">
                                Buy Target: {targetCurrency === 'INR' ? '₹' : '$'}{thresh.targetBuyPrice?.toLocaleString()}
                              </span>
                              <span className="text-[#6C63FF] text-[10px]">
                                (+{(((currentInTarget - (thresh.targetBuyPrice || 0)) / (thresh.targetBuyPrice || 1)) * 100).toFixed(1)}% to hit)
                              </span>
                            </>
                          )}
                        </div>
                  ...
                  <div className="flex flex-wrap items-center gap-6 md:border-l md:border-r border-[#D1D9E6] md:px-6">
                    {/* Price & Delta */}
                    <div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-3 shrink-0">
                    ...
                    <div className="text-left md:text-right">
                      ...
                      <p className="font-body text-[11px] text-[#6C63FF] font-medium mt-0.5 max-w-[200px] truncate">
                    ...
                    <button
                      ...
                      className="btn-neu-primary px-3.5 py-2 text-xs font-bold rounded-xl flex items-center gap-1 group-hover:shadow-neu-extrude transition-all"
                    ...
                    <button
                      ...
                      className="btn-neu w-8 h-8 rounded-xl flex items-center justify-center text-[#6C63FF] hover:-translate-y-0.5 active:translate-y-0.5 transition-all"
                    ...
                    <button
                      ...
                      className="btn-neu w-8 h-8 rounded-xl flex items-center justify-center text-[#E53E3E] hover:text-[#C53030] hover:-translate-y-0.5 active:translate-y-0.5 transition-all"
